---
title: 部署指南
description: 珞珈论坛预发与生产环境部署说明
---

# 部署指南

本页覆盖珞珈论坛当前仓库内已有的部署链路：`deploy/` 脚本 + GitHub Actions 工作流 + Docker Compose。
...
- 两套工作流共用并发锁 `whuforum-server`，避免同一台服务器并发部署冲突。
...
2. 服务器目录与脚本保持一致：
   - 生产仓库路径：`/opt/whuforum/WHUforum`
   - 预发仓库路径：`/opt/whuforum/WHUforum-staging`
3. 两个仓库目录下都已创建 `.env`（基于根目录 `.env.example`）。
4. 已配置反向代理（参考 `nginx/whuforum` 与 `nginx/whuforum-staging`）。
...
WEBSITE_URL=https://staging.forum.whu.edu.cn
NUXT_PUBLIC_API_BASE_URL=https://staging.forum.whu.edu.cn
NUXT_PUBLIC_WEBSOCKET_URL=https://staging.forum.whu.edu.cn/websocket
NUXT_PUBLIC_WEBSITE_BASE_URL=https://staging.forum.whu.edu.cn
```
...
```bash
# 生产（默认 main）
bash /opt/whuforum/WHUforum/deploy/deploy.sh

# 预发（默认 main）
bash /opt/whuforum/WHUforum/deploy/deploy_staging.sh
```

部署指定分支：

```bash
bash /opt/whuforum/WHUforum/deploy/deploy.sh feature/xxx
bash /opt/whuforum/WHUforum/deploy/deploy_staging.sh feature/xxx
```
...
```bash
docker compose -f /opt/whuforum/WHUforum/docker/docker-compose.yaml --env-file /opt/whuforum/WHUforum/.env ps
docker compose -f /opt/whuforum/WHUforum-staging/docker/docker-compose.yaml --env-file /opt/whuforum/WHUforum-staging/.env ps
```

查看核心服务日志：

```bash
docker compose -f /opt/whuforum/WHUforum/docker/docker-compose.yaml --env-file /opt/whuforum/WHUforum/.env logs -f springboot websocket-service frontend_service
```

本机健康检查（自动读取 `.env` 端口）：

```bash
ENV_FILE=/opt/whuforum/WHUforum/.env
...
2. 在服务器按分支重新部署：
   ...
   ```bash
   bash /opt/whuforum/WHUforum/deploy/deploy.sh rollback/2026-02-11
   ```

同理可用于预发：

```bash
bash /opt/whuforum/WHUforum/deploy/deploy_staging.sh rollback/2026-02-11
```
